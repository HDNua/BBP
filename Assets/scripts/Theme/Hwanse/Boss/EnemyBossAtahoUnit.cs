...
public class EnemyBossAtahoUnit : EnemyBossUnit
{
    ...
    IEnumerator CoroutineHokyokkwon()
    {
        ...
        Shot(shotPosition, destination, Bullet.Hokyukkwon, 1, SoundEffect.TigerCry);
    ...
    void Shot(Transform shotPosition, Vector3 destination, Bullet bulletIndex, int effectIndex, SoundEffect seIndex)
    {
        ...
        switch (bulletIndex)
        {
            case Bullet.Hokyukkwon:
                bulletUnit.FacingRight = FacingRight;
                Vector3 bulletPos = new Vector3(transform.position.x, shotPosition.position.y);
                Vector3 direction = destination - bulletPos;
                bulletUnit.MoveToDir(direction);
                //Debug.Log(direction);
                break;

            default:
                bulletUnit.FacingRight = FacingRight;
                bulletUnit.MoveToDst(destination);
                break;
        }
